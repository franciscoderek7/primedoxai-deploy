...
export default function DashboardClient({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!res.ok) {
      setError(data.upgrade_to ? `${data.error} Upgrade to ${data.upgrade_to} for more requests.` : data.error ?? "Request failed.");
      setLoading(false);
      return;
    }
      ...
      <div className="mx-auto max-w-3xl">
        ...
        <div className="mt-6 rounded-xl border border-white/10 bg-panel p-4">
          ...
          <div className="mt-2 h-2 w-full rounded-full bg-white/5">
            <div className="h-2 rounded-full bg-accent" style={{ width: `${usage.limit >= 999999 ? 5 : usagePct}%` }} />
          </div>
        </div>
        ...
        <div className="mt-6 rounded-xl border border-white/10 bg-panel p-4">
          ...
          <div className="mt-2 flex gap-2">
            <input readOnly value={referralLink} className="flex-1 rounded-lg border border-white/10 bg-ink px-3 py-2 text-sm text-gray-300" />
            <button onClick={copyReferral} type="button" className="rounded-lg bg-accent px-4 py-2 text-sm font-medium text-white hover:opacity-90">
              {copied ? "Copied" : "Copy"}
            </button>
          </div>
        </div>
