...
**OMNIGUARD.COM REPO CONFLICT — RESOLVED 2026-06-21:** Two repos both tried to claim `omniaguard.com` — legacy `franciscoderek7/omniaguard` (has real content) and new `franciscoderek7/omni-guard` (rebrand target, but `omni-guard-site/` is currently an **empty folder**, nothing built yet). Derek confirmed (2026-06-21): **keep the old repo live** until the rebrand is actually built — do not switch GitHub Pages custom domain to the new repo yet. Identity leaks found and fixed in the legacy `omniaguard-site/` (personal/parent-brand emails replaced with `enterprise@omniaguard.com` / `partnerships@omniaguard.com`). One leak remains unresolved: `free-scan.html`'s two "Book Full Audit" buttons still point to `paypal.me/franciscoderek7` (leaks Derek's personal PayPal handle on an anonymous Loop B brand) — not fixed because Claude will not invent a replacement payment handle without Derek explicitly confirming one is real and active.
